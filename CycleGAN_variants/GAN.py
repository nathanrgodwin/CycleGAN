...
if not os.path.isdir("./img/" + dataset + "_gan/"):
    os.makedirs("./img/" + dataset + '_gan/')

if not os.path.isdir("./log/" + dataset + "_gan/"):
    os.makedirs("./log/" + dataset + "_gan/")
    
if os.path.isfile("./log/" + dataset + "_gan/loss.log") and not load_partial_net:    
    os.remove("./log/" + dataset + "_gan/loss.log")
    
logfile = open("./log/" + dataset + "_gan/loss.log","a")
...
if (load_partial_net):
    if os.path.isfile("./model/" + dataset + "_gan/model_info.txt"):
        model_file = open("./model/" + dataset + "_gan/model_info.txt", "r")
        CURR_EPOCH = int(model_file.read())
        model_file.close()
    #G.load_state_dict(torch.load('./model/' + dataset + '/G.data'))
    F.load_state_dict(torch.load('./model/' + dataset + '_gan/F.data'))
    Dx.load_state_dict(torch.load('./model/' + dataset + '_gan/Dx.data'))
    #Dy.load_state_dict(torch.load('./model/' + dataset + '/Dy.data'))
    gan_opt.load_state_dict(torch.load('./model/' + dataset + '_gan/gan_opt.data'))
    dx_opt.load_state_dict(torch.load('./model/' + dataset + '_gan/dx_opt.data'))
...
for i in range(CURR_EPOCH,NUM_EPOCHS):
	...
	if int((10.0*j)/len(imageSet)) != j_prev:
        ...
        mapLoss = lossX_X + lossY_X# + lossX_Y_X + lossY_X_Y
        mapLoss.backward()
    ...
    #tv.save_image(src_x.data, './img/' + dataset + '_gan/src_x_%d.jpg'%(i))
    tv.save_image(src_y.data, './img/' + dataset + '_gan/src_y_%d.jpg'%(i))
    #tv.save_image(mapX_Y.data, './img/' + dataset + '_gan/mapX_Y_%d.jpg'%(i))
    tv.save_image(mapY_X.data, './img/' + dataset + '_gan/mapY_X_%d.jpg'%(i))
    gan_lr.step()
    dx_lr.step()
    #dy_lr.step()
    ...
    if not os.path.isdir("./model/" + dataset + "_gan/"):
        os.mkdir("./model/" + dataset + "_gan/")
    model_file = open("./model/" + dataset + "_gan/model_info.txt", "w")
    model_file.write("%d"%(i+1))
    model_file.close()
    #torch.save(G.state_dict(), './model/' + dataset + '_gan/G.data')
    torch.save(F.state_dict(), './model/' + dataset + '_gan/F.data')
    torch.save(Dx.state_dict(), './model/' + dataset + '_gan/Dx.data')
    #torch.save(Dy.state_dict(), './model/' + dataset + '_gan/Dy.data')
    torch.save(gan_opt.state_dict(), './model/' + dataset + '_gan/gan_opt.data')
    torch.save(dx_opt.state_dict(), './model/' + dataset + '_gan/dx_opt.data')
